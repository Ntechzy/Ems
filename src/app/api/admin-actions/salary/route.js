...
export async function GET(req, res) {
    try {
        ...
        if (!isAuthenticated || (isAuthenticated.role !== "super_admin")) {
        ...
        for (let employee of employees) {
            const leaveData = await Leave.aggregate([
                { $match: { user: employee.user_id, month } },
                {
                    $group: {
                        _id: "$user",
            ...
            const user_name = await userModel.findById(employee.user_id).select("name");
